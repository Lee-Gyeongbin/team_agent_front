...
const DEFAULT_NOTICE_MESSAGE = '네트워크 연결이 원활하지 않습니다.\n연결 상태를 확인한 후 다시 시도해 주세요.'
 ...
 */
const resolveGuideMessage = (): string => {
  ...
  if (networkIncidentMaintMessage.value) {
    return networkIncidentMaintMessage.value
  }
...
const applyNoticeOptions = (options: NetworkErrorNoticeOptions) => {
  message.value = options.message?.trim() || resolveGuideMessage()
  clearStatusMessage()
