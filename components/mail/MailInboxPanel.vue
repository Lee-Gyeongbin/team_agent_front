<template>
  ...
  <MailDetailModal
    ...
    sender-label="발신자"
    :sender-name="selectedMail?.fromName ?? ''"
    :sender-email="selectedMail?.from ?? ''"
    date-label="수신일"
    :date="selectedMail?.receivedDate ?? null"
    :body="selectedMail?.body ?? ''"
    :is-read="selectedMail?.isRead"
    @close="closeDetail"
  />
</template>
