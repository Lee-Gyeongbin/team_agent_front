...
/** A4처럼 세로가 더 긴 문서인지 — 캔버스를 가로 100%로 늘리면 잘리므로 별도 사이징 */
...
/** aspect-ratio를 캔버스에 직접 지정 — CSS 변수 상속/축약에 의존하지 않음 */
...
// coverImagePath 또는 reloadKey가 바뀔 때마다 presigned URL 재조회
// reloadKey: 경로가 동일한 채로 파일만 교체된 경우(재생성)에도 강제로 새 URL을 받아오기 위해 사용
watch([() => props.coverImagePath, () => props.reloadKey], ([newPath]) => loadCoverImage(newPath), { immediate: true })
