...
## 3. 착수 전 결정 (해당 도메인 웨이브 게이트)

- **D1 — Button `dark`/`primary-dark` 매핑**: `primary`? `secondary`? 혹은 라이브러리 재도입? (`primary-line`/`line-secondary`→`outline`은 확정) — **디자인 확인**. dark 쓰는 도메인(chat, meeting, library, guide) 웨이브 전제.
- **D2 — 처리 방식**: codemod 콜사이트 수정(추천, DS 단순화 유지) vs 라이브러리 하위호환 별칭. Modal `isOpen`은 48곳이라 라이브러리에 `open`이 `isOpen`도 허용하는 **한시적 alias**를 두면 W-Modal이 크게 줄어듦 — 검토.
- **D3 — Select `name` 6곳**: 폼 제출에 실제 필요한지 확인 → 제거 vs 라이브러리에 `name` 추가.
- **D4 — SCSS 정리 방식(도메인별)**: 라이브러리 새 마크업에 맞춰 재작성 vs 라이브러리 prop/slot으로 대체해 override 제거.
